  ...
  return (
    <section className="space-y-3 motion-safe:animate-[lab-fade-in_0.62s_ease-out]">
      ...
      <ul className="space-y-3">
        {items.map((item) => (
          <li key={`${item.tag}-${item.title}`} className="border-b border-[#e8dcc8] pb-4 last:border-b-0">
            <p className="mb-1 font-mono text-[11px] uppercase tracking-[0.16em] text-[#6a5f55]">{item.tag}</p>
            <Link href={item.url} className="group inline-block font-serif text-2xl text-[#1f1a16] hover:text-[#8f1f1f]">
              {item.title}
